...
class BotCommands(commands.Cog):
    ...
    async def game(self, ctx, prsn=None) -> None:
        ...
        for member in ctx.message.author.guild.members:
            ...
            if not prsn and not member.bot and game: 
                embed.add_field(
                    name=str(member).split("#")[0], value=game, inline=False
                )
            elif str(member).split("#")[0] == prsn and not member.bot and game:
                embed.add_field(
                    name=str(member).split("#")[0], value=game, inline=False
                )
    ...
    async def fry(self, ctx, res_factor1=4, res_factor2=5) -> None:
        ...
        print("Losing pixels")
        img = img.resize(
            (int(orig_size[0] / res_factor1), int(orig_size[1] / res_factor1)),
    ...
    async def letters(self, ctx, ls: str) -> None:
        ...
        if len(ls) > 20:
            await ctx.send(
                "Too many letters! Please use less than 20, I am just a little RaspberryPi"
            )
            return None
